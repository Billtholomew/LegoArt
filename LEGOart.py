...
def main():
    ...
    #pool = Pool()
    #args = [im[i*im.shape[0]/procs:(i+1)*im.shape[0]/procs,:,:] for i in xrange(procs)]
    #results = pool.map(dither,args)
    #print 'Procs:',procs,'Time:',time.clock()-start,'s'
    #for i in xrange(procs):
    #    im[i*im.shape[0]/procs:(i+1)*im.shape[0]/procs,:,:] = results[i]
